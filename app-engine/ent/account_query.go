...
type AccountQuery struct {
	...
	// eager-loading edges.
	withMembers *AccountMemberQuery
	withInvites *AccountInviteQuery
...
func (aq *AccountQuery) QueryMembers() *AccountMemberQuery {
	query := &AccountMemberQuery{config: aq.config}
...
func (aq *AccountQuery) QueryInvites() *AccountInviteQuery {
	query := &AccountInviteQuery{config: aq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := aq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (aq *AccountQuery) Clone() *AccountQuery {
	...
	return &AccountQuery{
		...
		withInvites: aq.withInvites.Clone(),
...
func (aq *AccountQuery) WithMembers(opts ...func(*AccountMemberQuery)) *AccountQuery {
	query := &AccountMemberQuery{config: aq.config}
...
func (aq *AccountQuery) WithInvites(opts ...func(*AccountInviteQuery)) *AccountQuery {
	query := &AccountInviteQuery{config: aq.config}
	for _, opt := range opts {
		opt(query)
	}
	aq.withInvites = query
	return aq
}
...
func (aq *AccountQuery) sqlAll(ctx context.Context) ([]*Account, error) {
	...
	if query := aq.withMembers; query != nil {
		...
		for i := range nodes {
			...
			nodes[i].Edges.Members = []*AccountMember{}
		}
		query.withFKs = true
		query.Where(predicate.AccountMember(func(s *sql.Selector) {
			s.Where(sql.InValues(account.MembersColumn, fks...))
		}))
		...
		for _, n := range neighbors {
			fk := n.account_members
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "account_members" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "account_members" returned %v for node %v`, *fk, n.ID)
			}
			node.Edges.Members = append(node.Edges.Members, n)
		}
	}

	if query := aq.withInvites; query != nil {
		fks := make([]driver.Value, 0, len(nodes))
		nodeids := make(map[int]*Account)
		for i := range nodes {
			fks = append(fks, nodes[i].ID)
			nodeids[nodes[i].ID] = nodes[i]
			nodes[i].Edges.Invites = []*AccountInvite{}
		}
		query.withFKs = true
		query.Where(predicate.AccountInvite(func(s *sql.Selector) {
			s.Where(sql.InValues(account.InvitesColumn, fks...))
		}))
		neighbors, err := query.All(ctx)
		if err != nil {
			return nil, err
		...
		for _, n := range neighbors {
			fk := n.account_invites
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "account_invites" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "account_invites" returned %v for node %v`, *fk, n.ID)
			}
			node.Edges.Invites = append(node.Edges.Invites, n)
		}
	}
