...
type UserQuery struct {
	...
	withMemberships *AccountMemberQuery
...
func (uq *UserQuery) QueryInvites() *AccountInviteQuery {
	query := &AccountInviteQuery{config: uq.config}
...
func (uq *UserQuery) QueryMemberships() *AccountMemberQuery {
	query := &AccountMemberQuery{config: uq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := uq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (uq *UserQuery) WithInvites(opts ...func(*AccountInviteQuery)) *UserQuery {
	query := &AccountInviteQuery{config: uq.config}
...
func (uq *UserQuery) WithMemberships(opts ...func(*AccountMemberQuery)) *UserQuery {
	query := &AccountMemberQuery{config: uq.config}
	for _, opt := range opts {
		opt(query)
	}
...
func (uq *UserQuery) sqlAll(ctx context.Context) ([]*User, error) {
	...
	if query := uq.withInvites; query != nil {
		...
		query.Where(predicate.AccountInvite(func(s *sql.Selector) {
			s.Where(sql.InValues(user.InvitesColumn, fks...))
		}))
		...
		for _, n := range neighbors {
			fk := n.user_invites
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "user_invites" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "user_invites" returned %v for node %v`, *fk, n.ID)
			}
	...
	if query := uq.withMemberships; query != nil {
		...
		for i := range nodes {
			fks = append(fks, nodes[i].ID)
			nodeids[nodes[i].ID] = nodes[i]
			nodes[i].Edges.Memberships = []*AccountMember{}
		}
		query.withFKs = true
